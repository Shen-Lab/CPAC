...
def cal_interaction(inputs, labels, functor, batch_size, prot_length, comp_length, logging=False, logpath=''):
    ...
    for i in range(N):
        if inputs[-1][i] != 0:
            ...
            '''
            # for case study
            idx_top10 = np.argsort(pred_label)[::-1][:10]
            idx_rc = [[i//length_comp+1, i%length_comp+1] for i in idx_top10]
            print(idx_rc)
            print(average_precision_score(true_label[idx_top10], np.ones(10)), idx_top10, true_label[idx_top10], pred_label[idx_top10], true_label.sum())
            # end case study
            '''
